...
* **Frontend:** o documento funcional fornecido pelo usuário não define nenhum Painel Administrativo para este serviço — "N/A" em toda História desta versão. **Exceção (2026-08-14):** a API de configuração `GET /admin/config` (API de Configuração — `arquitetura/decisoes/ADR-020-api-configuracao.md`) é a única superfície de UI desta versão, adotada por padrão organizacional (`padrao-desenvolvimento.md` seção 23), não por requisito funcional próprio — administra a configuração do serviço, transversal às Histórias, não uma delas.
